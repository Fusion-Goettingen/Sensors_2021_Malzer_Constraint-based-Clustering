...
def _tree_to_labels(X, single_linkage_tree, min_cluster_size=10,
					...
					cluster_selection_epsilon=0.0, prelabels=None, velocities=None, xy=None, directions=None):
    """Converts a pretrained tree and cluster size into a
    set of labels and probabilities.
    """
    condensed_tree, constraints = condense_tree(single_linkage_tree, prelabels, velocities, xy, directions, min_cluster_size)

    b3f_dict = {}
    stability_dict = compute_stability(condensed_tree)
    if prelabels is not None and not (allow_single_cluster == False and len(np.unique(prelabels)) == 0):
            b3f_dict = compute_b3f_measure(condensed_tree, prelabels, allow_single_cluster)

    labels, probabilities, stabilities, alternatives = get_clusters(condensed_tree,
                                                      stability_dict,
                                                      b3f_dict,
                                                      constraints,
    ...
    return (labels, probabilities, stabilities, condensed_tree, single_linkage_tree, alternatives)
...
def hdbscan(X, min_cluster_size=5, min_samples=None, alpha=1.0, cluster_selection_epsilon=0.0, prelabels=None, velocities=None, xy=None, directions=None,
            metric='minkowski', p=2, leaf_size=40,
    ...
    **kwargs : optional
    ...
    if prelabels is not None and len(prelabels) != X.shape[0]:
        raise ValueError('Number of labels must be equal to number of data points!')
    elif prelabels is None and cluster_selection_method == "b3f":
        raise ValueError('Cluster selection method "b3f" given but no labels supplied!')
    ...
    if velocities is not None and len(velocities) != X.shape[0]:
        raise ValueError('Number of velocities must be equal to number of data points!')
    ...
    if cluster_selection_method not in ('eom', 'leaf', 'b3f', 'e+constraint', 'constraint+e'):
        raise ValueError('Invalid Cluster Selection Method: %s\n'
                         'Should be one of: "eom", "leaf", "b3f", "e+constraint", "constraint+e"\n')
    ...
    return _tree_to_labels(X,
                           ...
                           allow_single_cluster,
                           match_reference_implementation,
						   cluster_selection_epsilon, prelabels, velocities, xy, directions) + \
...
class HDBSCAN(BaseEstimator, ClusterMixin):
    ...
    **kwargs : optional
    ...
    def __init__(self, min_cluster_size=5, min_samples=None, cluster_selection_epsilon=0.0, prelabels=None, velocities=None, xy=None, directions=None,
    ...
    def fit_predict(self, X, y=None):
        ...
        """
        self.fit(X)
        return (self.labels_, self.alternatives_)
